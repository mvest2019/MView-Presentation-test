 ...
 *
 * HEIGHT IS ITS OWN, NOT THE GRID'S. `self-start` opts out of `items-stretch`, so
 * the form growing on a failed submit cannot drag this card with it — its content
 * never changes, so neither should its height. The `min-h` then puts it back level
 * with the form at rest: the form settles at 510px from ~1071px up, where the grid
 * reaches its 1015px max width, and the card's own content is only 459px.
 *
 * The number is measured, and it is the compromise in this design: below ~1071px
 * the form is narrower and taller (530px at 900px wide), so the two cards stop
 * ending on exactly the same line there. Reserving space for the error messages
 * instead would keep them locked together at every width, but it spaces the fields
 * ~17px further apart at rest, which the design does not want.
 *
 ...
 *  - the rows are a `justify-between` track with `gap-[18px]` as a floor. That
 *    track is what spreads the spare height from `min-h` across the rows, and the
 *    gap matters on phones, where there is no spare height to distribute and
 *    `justify-between` alone would let the rows collapse against each other.
 */
export function ContactInfo() {
  return (
    <div className="flex flex-col self-start rounded-mv border border-mv-line bg-mv-card p-[22px] shadow-mv min-[861px]:min-h-[510px]">
