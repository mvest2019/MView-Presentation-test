 ...
 * saved density is read from `localStorage` in an effect, so the very first
 * paint is Essentials. That is the reference's own behaviour — it pre-paints the
 * Essentials default precisely "so first paint never flashes Detailed", then
 * re-applies the stored choice. Reading storage during render is not an option
 * on a server-rendered page, and guessing would flash the wrong density for
 * every owner who never changed it.
 */
...
/**
 * The fallback shown while `useSearchParams` suspends during a prerender.
 *
 * It is the same `<div>` with the same default classes the provider would pick
 * for a visitor arriving with no parameters, so the swap is invisible rather
 * than a flash of unstyled portal.
 */
export function PortalRootFallback({ children }: { children: ReactNode }) {
  return (
    <div
      className={`mv-portal ${FUNNEL_STATE_CLASS[DEFAULT_FUNNEL_STATE]} ${viewTierClasses(DEFAULT_VIEW_TIER)}`}
      data-funnel-state={DEFAULT_FUNNEL_STATE}
      data-view-tier={DEFAULT_VIEW_TIER}
    >
      {children}
    </div>
  );
